...
class WBlock2(nn.Module):
    def __init__(self, config, layer_idx,scale=False):
        ...
        self.model=LlamaDecoderLayer(config,layer_idx)
...
class LMNetForSequenceClassification(LlamaPreTrainedModel):
    def __init__(self, config,layer_list=[1,2,1],pl=0):
        ...
        t=0
        for i in range(self.layer_list[0]):
            self.ws.append(WBlock_mlp(config))
            t+=1
        for l in range (self.layer_num-1):
            self.lsid.append(t)
            #t+=int(self.layer_list[l]*self.layer_list[l+1])
    ...
    def forward(
        ...
        hsl_old=[]
        for i in range(self.layer_list[0]):
            a,p=self.ws[i](hidden_states,attention_mask=attention_mask)
            pid+=1
...
class LMNetForSequenceClassificationS(LlamaPreTrainedModel):
    def __init__(self, config,layer_list=[1,2,1],pl=0):
        super().__init__(config,layer_list=[1,2,1],pl=0)
