...
class Attention0(nn.Module):
    ...
    def _attn(self, q, k, v, len_kv=None,attention_mask=0):
        w = torch.matmul(q, k)
        if self.scale:
    ...
    def forward(self, x, history=None, layer_past=None, len_past=None,attention_mask=0):
        hidden_states = x
        ...
        a = self._attn(query, key, value, len_kv = len_kv,attention_mask=attention_mask)
...
class WBlock(nn.Module):
    ...
    def forward(self, x,attention_mask=None,past=None, len_past=None):
        layer_past=None
        if past is not None:
            layer_past=past[0]
        a, present = self.attn(x,layer_past=layer_past, len_past=len_past,attention_mask=attention_mask)
        x = x + a
...
class WBlock2(nn.Module):
    def __init__(self, config, layer_idx,scale=False):
        super(WBlock2, self).__init__()
        self.model=Qwen2DecoderLayer(config,layer_idx)
    ...
    def forward(self, x,attention_mask=0,position_embeddings=None,past=None, len_past=None):
        past_seen_tokens=0
...
class WBlock_set(nn.Module):
    def __init__(self, config, scale=False):
        super(WBlock_set, self).__init__()
        nx = config.hidden_size
        self.mlp_w = MLP(nx,nx)
        self.mlp_b = MLP(nx,nx)
        self.mlp2 = MLP(nx,nx)
        

    def forward(self, x,attention_mask):
        am=attention_mask.unsqueeze(-1)#bz,512,1
        l=torch.sum(am,1,True)
        w=self.mlp_w(x)*am
        b=self.mlp_b(x)*am
        w=torch.sum(w,1,True)/l
        b=torch.sum(b,1,True)/l#bz
        
        x_mod=x*(1+w)+b
        x=self.mlp2(x_mod)
        return x+x_mod,None
    
class WBlock1(nn.Module):
    def __init__(self, config, layer_idx,scale=False):
        super(WBlock1, self).__init__()
        self.model=Qwen2DecoderLayer(config,layer_idx)
        self.fc=nn.Linear(config.hidden_size,config.hidden_size)
        self.config=config
    ...
    def forward(self, hidden_states: torch.Tensor,
        ...
        causal_mask=self._update_causal_mask(attention_mask,hidden_states,cache_position,None,self.config.output_attentions)
        layer_outputs = self.model(
...
class Qwen2Attention(nn.Module):
    ...
    def forward(
        ...
        if past_key_value is not None:
            ...
            key_states, value_states = past_key_value.update(key_states, value_states, self.layer_idx+lid_bias, cache_kwargs)
...
class Qwen2Model(Qwen2PreTrainedModel):
    ...
    def forward(
        ...
        if cache_position is None:
            past_seen_tokens = past_key_values.get_seq_length() if past_key_values is not None else 0
            cache_position = torch.arange(
                past_seen_tokens, past_seen_tokens + inputs_embeds.shape[1]+pl, device=inputs_embeds.device
            )
        ...
        #print(len(next_cache),len(next_cache[0]))#(24,2)
...
class LMNetForCausalLM(Qwen2PreTrainedModel, GenerationMixin):
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config,layer_list=[1,4,4,4,1],pl=0):
        super().__init__(config,layer_list=[1,4,4,4,1],pl=0)
        self.model = Qwen2Model(config)
        self.vocab_size = config.vocab_size
        self.lm_head = nn.Linear(config.hidden_size, config.vocab_size, bias=False)
        ...
        for l in range(self.layer_num-1):
            ...
            for j in range(self.layer_list[l+1]):
                ...
                for i in range(self.layer_list[l]):
                    ...
                    cache_position=cache_position,
                    position_embeddings=pe)
                    if l==self.layer_num-2:
                        aw=out[1][0]#14, 512, 512
                        awout.append(aw.mean(0))
                        #print(aw.size())#[1, 14, 512, 512])
                    a=out[0]
                    #if use_cache:
                    #    p = out[2 if output_attentions else 1]
...
class LMNetForCausalLMS(Qwen2PreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config,layer_list=[1,2,1],pl=0):
        ...
        for i in range(self.num_models):
            self.ms.append(Qwen2Model(config))
        self.lm_head = nn.Linear(config.hidden_size, config.vocab_size, bias=False)
...
class LMNetForSequenceClassification(Qwen2PreTrainedModel):
    def __init__(self, config,layer_list=[1,2,1],pl=32):
        ...
        #self.ws.append(WBlock_set(config))
        #self.ws.append(WBlock2(config,t+config.num_hidden_layers))#(GPT2Block(config))#(WBlock( self.edge_config,config))
        #self.ws.append(WBlock( self.edge_config,config))
        #self.ws.append(WBlock( self.edge_config,config))
    ...
    def forward(
        ...
        hsl_old=[]
        for i in range(self.layer_list[0]):
            a,p=self.ws[i](hidden_states,attention_mask=attention_mask,position_embeddings=pe)
            pid+=1
            ...
            hs=outputs[0]
            p=outputs[1]
            #print(len(p),len(p[0]))#12,2
            #print(p[0][0].size())#torch.Size([8, 12, 544, 64])
            hs0=hidden_states
            pid+=1
            present_list.append(p)
            hsl_old.append(hs)
        ...
        #hidden_states=self.ws[-1](hidden_states,attention_mask=attention_mask)[0]
        #pid+=1
        #present_list.append(p)

        '''hidden_states,p=self.ws[-2](hidden_states,tam)
        pid+=1
        present_list.append(p)

        hidden_states,p=self.ws[-3](hidden_states,tam)
        pid+=1
        present_list.append(p)'''
        ...
        return SequenceClassifierOutputWithPast(
            ...
            hidden_states=None,#hidden_states[:,self.pl:],
            attentions=None,
        )
...
class LMNetForSequenceClassificationS(Qwen2PreTrainedModel):
    def __init__(self, config,layer_list=[1,2,1],pl=0):
        super().__init__(config,layer_list=[1,2,1],pl=0)
        self.ms=nn.ModuleList([])
        self.num_labels = config.num_labels
        ...
        if self.pl>0:
            self.pfx = torch.nn.Parameter((torch.randn(1,self.pl,self.config.hidden_size)*0.1437+0.0004).cuda(), requires_grad=True)
    ...
    def forward(
        ...
        if self.pl>0:
            if p_list[pid]==None:
                #print(hidden_states.size(),self.pfx.size(),self.pfx.repeat(hidden_states.size(0),1,1))
                hidden_states=torch.cat((self.pfx.repeat(hidden_states.size(0),1,1),hidden_states),1)
            attention_mask=torch.cat([torch.ones((attention_mask.size(0),self.pl),device=self.ms[0].device),attention_mask],-1).to(self.ms[0].device)
        batch_size=hidden_states.size(0)
        input_shape=hidden_states.size()[:-1]
        ...
        '''hidden_states=self.ws[-1](hidden_states,attention_mask=attention_mask)[0]
        #hidden_states,p=self.ws[-1](hidden_states,attention_mask=attention_mask,position_embeddings=pe)
        pid+=1
        present_list.append(p)'''
