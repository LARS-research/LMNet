...
def _reorder_cache0(past: Tuple, beam_idx: Tensor) -> Tuple[Tensor]:
    return tuple(layer_past.index_select(1, beam_idx).contiguous().detach() for layer_past in past)

def _reorder_cache(past: Tuple, beam_idx: Tensor) -> Tuple[Tensor]:
    out=[]
    for i in past:
        out.append(tuple(layer_past.index_select(1, beam_idx).contiguous().detach() for layer_past in i)) 
    return out
...
def beam0(model, data_iter, args):
    model.eval()
    total_loss = 0.
    start_time = time.time()
    ...
    with torch.no_grad():
        for idx, data in enumerate(data_iter):
            ...
            past = None
            len_past = None
            #print(_query.size())#1,512
            #print(_query,_query[0][_query_len-2],_query[0][_query_len-1],_query[0][_query_len])
            #_query[0][_query_len-1]=eos
            _query = _query.repeat(1, num_beams).view(batch_size * num_beams, -1)
            _query_len = _query_len.unsqueeze(-1).repeat(1, num_beams).view(-1)
            #print(_query.size(),_query_len.size())#torch.Size([10, 512]) torch.Size([10])
            ...
            with torch.no_grad():
                for i in range(0, args.eval_len):
                    ...
                    logits, past = model(_query) 
                    #print(len(past),past[0].size())24 torch.Size([2, 10, 16, 512, 64])
                    #print(logits.size())#torch.Size([10, 512, 50257])
                    logits = logits[_bbatch, (_query_len-1).long(), :] # batch_size * beam, vocab
                    #print(logits.size())#torch.Size([10, 50257])
                    ...
                    #######
                    _query=torch.cat([_query[:,:_query_len[0]],token_id,_query[:,_query_len[0]:-1]],-1)
                    _query_len=_query_len+1
...
if __name__ == '__main__':
    ...
    lm_net = LMNet([1,2,1],config,config.n_ctx,pl=32)
    if args.init_checkpoint is not None:
        print('loading model pretrained weight.')
        #cp = torch.load(args.init_checkpoint, map_location=torch.device('cpu'))
        #lm_net.load_weight(cp)  
        #print(torch.load(args.init_checkpoint).keys())
        lm_net.node.load_weight(torch.load('/home/wushiguang/LoRA/examples/NLG/pretrained_checkpoints/gpt2-medium-pytorch_model.bin')) 
        
        t=torch.load(args.init_checkpoint)
        lm_net.load_state_dict(t['model_state_dict'])
        #lm_net.ws.load_state_dict(t['model_state_dict'])
        '''sd=torch.load(args.init_checkpoint)['model_state_dict']
        sd1={}
        for k in sd.keys():
            sd1[k[7:]]=sd[k]
        lm_net.load_state_dict(sd1)  '''
    lm_net = lm_net.cuda()
