...
from model_net import GPT2Config, LMNet
...
parser.add_argument('--eval_interval', type=int, default=10000, help='eval interval')
...
def train_validate(
    ...
    for idx, data in enumerate(train_loader):
        ...
        _input = data['input'].to(args.device)
        _target = data['target'].to(args.device)
        _msk = data['mask'].to(args.device)#8,512

        _lm_logits, _lm_loss = model(
            _input, lm_labels=_target, lm_mask=_msk, label_smooth=args.label_smooth
        ) 
        #print(_lm_logits.size())[#8, 512, 50257]
        #print(_lm_loss)
        ...
        if train_step % args.save_interval == 0: 
            if args.rank == 0:
                model_path = os.path.join(args.work_dir, f'model.{train_step}.cc.pt')
                print('saving checkpoint', model_path)
                torch.save({'model_state_dict': model.module.state_dict()}, model_path)
                #torch.save({'model_state_dict': model.module.ws.state_dict()}, model_path)
...
if __name__ == '__main__':
    ...
    lm_net = LMNet([1,2,1],config,config.n_ctx)
    if args.init_checkpoint is not None:
        print('loading model pretrained weight.')
        lm_net.node.load_weight(torch.load(args.init_checkpoint))    

    lm_net = lm_net.cuda()
    #print(lm_net.state_dict()['node.lm_head.decoder.weight'].std(),lm_net.state_dict()['node.lm_head.decoder.weight'].mean())
    
    #if args.lora_dim > 0:
    #    lora.mark_only_lora_as_trainable(lm_net)
    #num_params = sum(param.numel() for param in lm_net.ws.parameters())
    #print(lm_net.ws.state_dict().keys(),num_params)
